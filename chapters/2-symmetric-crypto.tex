...
$j$ (z.B. ist $i$ die Blocknummer auf der Harddisk und $j$ der entsprechende Sektor) und ein $\alpha$, das ein primitives Element in $GF(2^{128})$ ist. 
...
\begin{itemize}
    \item ideal für Datenverschlüsselung an Endpunkten (Festplattenverschlüsselung)
...
\subparagraph{Konfusion} \index{Konfusion}\index{Blockcipher!Konfusion} heißt, die Beziehung zwischen Klartext, Schlüssel und Ciphertext 
soll so komplex wie möglich sein.
\subparagraph{Diffusion} \index{Diffusion}\index{Blockcipher!Diffusion} heißt, der Ciphertext hängt von so vielen Klartext Bits ab wie nur möglich. \\
...
DES wird mittels Permutationen und Substitutionen realisiert, die 16 Mal (als ``Runden'') wiederholt werden, siehe Abb. \ref{fig:des-round}. Diese Struktur eignet sich 
...
(``Compression Permutation''\index{Compression Permutation}) (Tabelle \ref{tab:key-tranformation-4}):
...
In jeder Runde wird die Hälfte $R_i$ (32 Bit) mit dem 48 Bit Key xor-t werden. Dafür wird die Expansion Permutation \index{Expansion Permutation} benötigt. Hier gibt
...
\begin{table}[h]
    ...
    \begin{tabular}{|*{12}{c|}}
        ...
        \hline 
    \end{tabular}
    \caption{(Expansion Permutation) Output-Array nach Expansion Permutation}
\end{table}

\begin{itemize}
    \item Der Datenblock muss auf Schlüsselgröße erweitert werden
    \item Durch Wiederholung einzelner Bits breitet sich Einfluss des Inputs auf den Output schneller aus (ein Inputbit beeinflusst 2 S-Box Lookups)
\end{itemize}
...
\paragraph{SubBytes} ist die einzige nicht-lineare Operation in AES. %(? Todo: \verb|folien AK_02_block_v02.pdf|, Seite 65)

\begin{itemize}
    \item Konfusion\index{Konfusion}
    \item S-Box 
...
\begin{table}[h]
    ...
    \begin{tabular}{|*{4}{c|}}
        \hline
        % Row 1 
        \cellcolor{red-1}0 & \cellcolor{yellow-1}4 & \cellcolor{green-1}8 & \cellcolor{cyan-1}12 \\
        \hline 
        % Row 2 
        \cellcolor{red-1}1 & \cellcolor{yellow-1}5 & \cellcolor{green-1}9 & \cellcolor{cyan-1}13 \\
        \hline 
        % Row 3 
        \cellcolor{red-1}2 & \cellcolor{yellow-1}6 & \cellcolor{green-1}10 & \cellcolor{cyan-1}14 \\
        \hline 
        % Row 4 
        \cellcolor{red-1}3 & \cellcolor{yellow-1}7 & \cellcolor{green-1}11 & \cellcolor{cyan-1}15 \\
        \hline 
    \end{tabular}\quad
    \begin{tabular}{|*{4}{c|}}
        \hline
        ...
        \hline 
        % Row 2 
        \cellcolor{yellow-1}5 & \cellcolor{green-1}9 & \cellcolor{cyan-1}13 & \cellcolor{red-1}1 \\
        \hline 
        % Row 3 
        \cellcolor{green-1}10 & \cellcolor{cyan-1}14 & \cellcolor{red-1}2 & \cellcolor{yellow-1}6 \\
        \hline 
        % Row 4 
        \cellcolor{cyan-1}15 & \cellcolor{red-1}3 & \cellcolor{yellow-1}7 & \cellcolor{green-1}11 \\
        \hline 
    \end{tabular}
    \caption{(ShiftRows) Array vor und nach der Operation.}
\end{table}
...
\begin{table}[h]
    ...
    \begin{tabular}{|*{4}{c|}}
        ...
        \hline 
    \end{tabular}
    \caption{(MixColumns) Einträge des States}
\end{table}
...
Notation: statt $a_{i,j}$ steht im Folgenden $a_i$, zur Wahrung der Übersichtlichkeit, weil die Einträge eben spaltenweise ausgelesen werden. \\

\paragraph{Beispiel}: Stelle das Polynom
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Volle Diffusion\index{Diffusion} nach 2 Runden (mit Feistelnetzwerk nicht erreichbar)
...
\noindent Bei Streamciphern sind Konfusion\index{Konfusion} und Diffusion\index{Diffusion} nicht möglich (sic! Vgl. ChaCha20). Das heißt die Sicherheit liegt allein in 
...
\noindent Die Runden, die in der Berechnung durchlaufen werden, sorgen für eine gute Diffusion\index{ChaCha20!Diffusion}. \\
...
\noindent Zwei Nachrichten mit demselben Key dürfen nicht den gleichen Ciphertext erzeugen. Der Counter wird pro 64-Byte-Block erhöht. \\

\noindent Output: Keystream\index{Schlüsselstrom!ChaCha20}\index{Keystream!ChaCha20}, der aus Key, Nonce und Counter erzeugt wird und mit dem Plaintext verxort den Ciphertextstream ergibt.\\
